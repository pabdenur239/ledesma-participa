...
CATEGORIA_TEMATICA = "internacional"


def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["bbc_mundo"]


class BBCMundoRSSCollector(Collector):
    """Collector RSS real de BBC Mundo (https://feeds.bbci.co.uk/mundo/rss.xml).

    Fuente de noticias internacionales en español. No asigna localidad; no
    usa boilerplate estilo WordPress (BBC tiene su propio dialecto RSS, sin
    ese párrafo). Requiere acceso saliente a internet; no se ejecuta durante
    las pruebas automáticas, que usan un fixture XML local en su lugar.
    """
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"BBC Mundo: {error}") from error
