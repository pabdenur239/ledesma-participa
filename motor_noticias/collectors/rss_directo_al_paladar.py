...
CATEGORIA_TEMATICA = "gastronomia"


def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["directo_al_paladar"]


class DirectoAlPaladarRSSCollector(Collector):
    """Collector RSS real de Directo al Paladar
    (https://www.directoalpaladar.com/feedburner.xml).

    Fuente de gastronomía/recetas en español (Webedia). No asigna
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"Directo al Paladar: {error}") from error
