...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["el_gourmet"]


class ElGourmetRSSCollector(Collector):
    """Collector RSS real de El Gourmet (https://elgourmet.com/rss).
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"El Gourmet: {error}") from error
