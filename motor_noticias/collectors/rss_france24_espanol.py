...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["france24_espanol"]


class France24EspanolRSSCollector(Collector):
    """Collector RSS real de France 24 en español (https://www.france24.com/es/rss).

    Fuente de noticias internacionales en español. No asigna localidad.
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"France 24 Español: {error}") from error
