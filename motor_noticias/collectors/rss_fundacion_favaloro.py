...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["fundacion_favaloro"]


class FundacionFavaloroRSSCollector(Collector):
    """Collector RSS real de Fundación Favaloro
    (https://www.fundacionfavaloro.org/feed/).

    Fuente médica institucional argentina confiable (se eligió sobre
    Hospital Italiano porque este último no tiene RSS público verificado;
    ver informe de fuentes descartadas). No asigna localidad. Requiere
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"Fundación Favaloro: {error}") from error
