...
class ErrorRecoleccionJujuyAlDia(RuntimeError):
    """Error controlado al recolectar el RSS de Jujuy al día."""


def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["jujuyaldia"]
...
def parsear_rss(contenido: Union[str, bytes], nombre_fuente: str) -> List[dict]:
    """Parsea el feed RSS real de Jujuy al día (WordPress estándar). La
    ...
    se publicó primero en Jujuy al día." que se descarta antes de limpiar
    el resto del HTML."""
...
class JujuyAlDiaRSSCollector(Collector):
    """Collector RSS real de Jujuy al día (https://www.jujuyaldia.com.ar/feed/).

    Fuente provincial que publica habitualmente avisos de servicios
    ("Atención vecinos de...") específicos de Fraile Pintado, Calilegua y
    Yuto: no asigna una localidad fija, la relevancia geográfica de cada
    noticia se deriva de su contenido con el clasificador existente.
    ...
    def recolectar(self) -> List[dict]:
        ...
        try:
            ...
            raise ErrorRecoleccionJujuyAlDia(
                f"Jujuy al día respondió HTTP {error.code} ({error.reason}) al pedir {self.url}"
            ) from error
        except urllib.error.URLError as error:
            raise ErrorRecoleccionJujuyAlDia(
                f"No se pudo conectar a Jujuy al día ({self.url}): {error.reason}"
            ) from error
