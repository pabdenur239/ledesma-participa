...
class ErrorRecoleccionJujuyGrafico(RuntimeError):
    """Error controlado al recolectar el RSS de Jujuy Gráfico."""


def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["jujuygrafico"]
...
def parsear_rss(contenido: Union[str, bytes], nombre_fuente: str) -> List[dict]:
    """Parsea el feed RSS real de Jujuy Gráfico (WordPress estándar, mismo
    formato que Jujuy al día/Canal 6 Libertador: ver rss_jujuyaldia.py). La
    imagen no viene en un <enclosure> separado: está embebida como un
    <img src="..."> al principio del HTML de <description>. La descripción
    también trae, al final, el párrafo estándar de WordPress "La entrada X
    se publicó primero en Y." que se descarta antes de limpiar el resto del
    HTML."""
...
class JujuyGraficoRSSCollector(Collector):
    """Collector RSS real de Jujuy Gráfico (https://jujuygrafico.com.ar/feed/).

    Medio provincial que declara explícitamente cobertura del "Ramal
    jujeño" (la región que incluye Libertador General San Martín y el
    Departamento Ledesma) además de la capital, la Quebrada y la Puna: no
    asigna una localidad fija, la relevancia geográfica de cada noticia se
    deriva de su contenido con el clasificador existente — mismo criterio
    que Jujuy al día/Somos Jujuy/TodoJujuy, que ya aportan noticias locales
    y departamentales reales por este mismo mecanismo. Requiere acceso
    saliente a internet; no se ejecuta durante las pruebas automáticas, que
    usan un fixture XML local en su lugar.
    """
    ...
    def recolectar(self) -> List[dict]:
        ...
        try:
            ...
            raise ErrorRecoleccionJujuyGrafico(
                f"Jujuy Gráfico respondió HTTP {error.code} ({error.reason}) al pedir {self.url}"
            ) from error
        except urllib.error.URLError as error:
            raise ErrorRecoleccionJujuyGrafico(
                f"No se pudo conectar a Jujuy Gráfico ({self.url}): {error.reason}"
            ) from error
