...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["ministerio_salud_argentina"]


class MinisterioSaludArgentinaRSSCollector(Collector):
    """Collector RSS real del Ministerio de Salud de la Nación Argentina
    (https://www.argentina.gob.ar/salud/noticias/rss).

    Fuente institucional oficial de salud pública argentina. No asigna
    localidad. Requiere acceso saliente a internet; no se ejecuta durante
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"Ministerio de Salud de la Nación: {error}") from error
