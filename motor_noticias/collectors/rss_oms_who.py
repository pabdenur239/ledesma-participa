...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["oms_who"]


class OMSWHORSSCollector(Collector):
    """Collector RSS real de la OMS/WHO, edición en español
    (https://www.who.int/rss-feeds/news-spanish.xml).

    Fuente institucional oficial de salud pública global. No asigna
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"OMS/WHO: {error}") from error
