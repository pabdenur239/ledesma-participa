...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["ops_paho"]


class OPSPAHORSSCollector(Collector):
    """Collector RSS real de la OPS/PAHO (https://www.paho.org/es/rss.xml).

    Fuente institucional oficial de salud pública regional (Organización
    Panamericana de la Salud, oficina regional de la OMS). No asigna
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"OPS/PAHO: {error}") from error
