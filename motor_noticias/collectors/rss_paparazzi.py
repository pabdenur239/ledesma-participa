...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["paparazzi"]


class PaparazziRSSCollector(Collector):
    """Collector RSS real de Paparazzi (https://www.paparazzi.com.ar/feed/).

    Fuente de espectáculos/chimentos de Argentina. No asigna localidad: es
    contenido temático, no geográfico. Requiere acceso saliente a internet;
    no se ejecuta durante las pruebas automáticas, que usan un fixture XML
    local en su lugar.
    """
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"Paparazzi: {error}") from error
