...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["paulina_cocina"]


class PaulinaCocinaRSSCollector(Collector):
    """Collector RSS real de Paulina Cocina (https://www.paulinacocina.net/feed/).

    Fuente de gastronomía/recetas en español. No asigna localidad. Requiere
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"Paulina Cocina: {error}") from error
