...
class ErrorRecoleccionSomosJujuy(RuntimeError):
    """Error controlado al recolectar el RSS de Somos Jujuy."""


def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["somos_jujuy"]
...
def parsear_rss(contenido: Union[str, bytes], nombre_fuente: str) -> List[dict]:
    """Parsea el feed RSS real de Somos Jujuy. A diferencia de TodoJujuy, la
    imagen no viene en un <enclosure> separado: está embebida como un
    <img src="..."> al principio del HTML de <description>, seguida de un
    <p>resumen</p>. Se extrae la imagen del <img> y se limpia el resto del
    HTML para quedarse solo con el texto del resumen."""
    ...
    for item in raiz.findall("./channel/item"):
        ...
        noticias.append(
            {
                "titulo": titulo,
                "texto": _quitar_etiquetas_html(descripcion_html),
                "url": enlace,
...
class SomosJujuyRSSCollector(Collector):
    """Collector RSS real de Somos Jujuy (https://www.somosjujuy.com.ar/feed/).
    ...
    def recolectar(self) -> List[dict]:
        ...
        try:
            ...
            raise ErrorRecoleccionSomosJujuy(
                f"Somos Jujuy respondió HTTP {error.code} ({error.reason}) al pedir {self.url}"
            ) from error
        except urllib.error.URLError as error:
            raise ErrorRecoleccionSomosJujuy(
                f"No se pudo conectar a Somos Jujuy ({self.url}): {error.reason}"
            ) from error
