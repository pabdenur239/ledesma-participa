...
def _cargar_config(path: Optional[Path] = None) -> dict:
    with open(path or CONFIG_PATH_DEFAULT, encoding="utf-8") as f:
        return json.load(f)["tmz"]


class TMZRSSCollector(Collector):
    """Collector RSS real de TMZ (https://www.tmz.com/rss.xml).

    Fuente de celebridades/espectáculos internacional, en inglés (no hay
    versión en español con feed propio). No asigna localidad. Requiere
    ...
    def recolectar(self) -> List[dict]:
        try:
            ...
            raise ErrorRecoleccionRSSGenerico(f"TMZ: {error}") from error
