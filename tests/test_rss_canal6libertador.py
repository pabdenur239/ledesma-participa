...
from motor_noticias.collectors.rss_canal6libertador import (
...
FIXTURE_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "fixtures" / "canal6libertador_rss_prueba.xml"
)
NOMBRE_FUENTE = "Canal 6 Libertador"

TITULO_CONCEJO = "El Concejo Deliberante de Libertador aprobó el presupuesto 2027"
TITULO_RECLAMO = "Vecinos de barrio Norte reclaman por el estado de la calle principal"
TITULO_PROVINCIAL = "El gobierno de Jujuy anunció nuevas obras hídricas en la provincia"
...
class TestParsearRSS(unittest.TestCase):
    ...
    def test_extrae_las_noticias_reales_incluido_el_duplicado(self):
        ...
        self.assertIn(TITULO_RECLAMO, titulos)
    ...
    def test_extrae_titulo_resumen_url_fecha_y_fuente(self):
        ...
        self.assertEqual(
            concejo["texto"],
            "El cuerpo legislativo municipal dio media sanción al presupuesto que regirá el "
            "próximo año en Libertador General San Martín.",
        )
        self.assertEqual(
            concejo["url"],
            "https://canalseis.com.ar/2026/08/17/concejo-deliberante-libertador-presupuesto-2027/",
        )
        self.assertEqual(concejo["fecha"], "Mon, 17 Aug 2026 11:00:00 +0000")
    ...
    def test_extrae_imagen_embebida_en_la_descripcion(self):
        ...
        self.assertEqual(
            concejo["imagen_url"],
            "https://canalseis.com.ar/wp-content/uploads/2026/08/concejo.jpg",
        )
        self.assertNotIn("<img", concejo["texto"])
        self.assertNotIn("<p>", concejo["texto"])

    def test_imagen_vacia_cuando_la_descripcion_no_trae_img(self):
        noticias = parsear_rss(self.contenido, NOMBRE_FUENTE)
        reclamo = next(n for n in noticias if n["titulo"] == TITULO_RECLAMO)
        self.assertIsNone(reclamo["imagen_url"])
...
class TestPipelineConFixtureRSS(unittest.TestCase):
    ...
    def test_noticia_local_de_libertador_queda_preparada_y_urgente(self):
        ...
        self.assertEqual(noticia_concejo.territorio, "local")
        self.assertTrue(noticia_concejo.urgente)
...
class TestPeticionHTTP(unittest.TestCase):
    ...
    @patch("motor_noticias.collectors.rss_canal6libertador.urllib.request.urlopen")
    def test_usa_request_con_headers_en_lugar_de_url_cruda(self, urlopen_mock):
        ...
        collector = Canal6LibertadorRSSCollector()
    ...
    def test_http_error_se_convierte_en_error_controlado(self, urlopen_mock):
        urlopen_mock.side_effect = urllib.error.HTTPError(
            url="https://canalseis.com.ar/feed/",
            code=403,
            msg="Forbidden",
            hdrs=None,
            fp=None,
        ...
        with self.assertRaises(ErrorRecoleccionCanal6Libertador) as contexto:
    ...
    def test_url_error_se_convierte_en_error_controlado(self, urlopen_mock):
        ...
        with self.assertRaises(ErrorRecoleccionCanal6Libertador):
            collector.recolectar()
